...
- **Auth:** email is auto-confirmed (DB trigger) and signup auto-signs-in; the splash can never hang (timeouts + globalThis Supabase singleton). **Saving profile/preferences refreshes silently** (`loadProfile(uid, { silent: true })`): a normal refresh would flip `profileLoaded` → `AuthGate` shows the splash → the whole app remounts, bouncing the user off Profile/Settings and dropping the "Saved" toast. Any post-save profile reload must stay silent.
